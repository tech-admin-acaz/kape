...
/**
 * API route to fetch precipitation statistics for a given location.
 */
export async function GET(
    ...
    const apiPath = `${API_BIO_URL}/graph/pre/${apiTypePath}/${id}/${model}/${scenario}`;
    console.log(`Buscando estatísticas de precipitação em: ${apiPath}`);
    ...
    try {
        ...
        if (!response.ok) {
            ...
            console.error(`Error fetching precipitation stats from external API (${apiPath}):`, response.status, errorText);
        ...
        console.error(`Error fetching precipitation stats data from ${apiPath}:`, error);
